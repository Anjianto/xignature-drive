<template>
  <PageTab :is-loading="isLoading">
    <PageTabGroup>
      ...
      <DatatableWrapper
        v-if="PaymentMethods"
        :table-data="{ data: PaymentMethods }"
        :paginator="false"
        :columns="columns"
        class="table"
      >
        ...
        <template slot-scope="{ row }">
          <tr
            :class="{
              'is-deleting': row.data.attributes.card_id === deletingID,
            }"
          >
              ...
              <span class="cell-item">
                <div class="credit-card">
                  <img
                    ...
                    :src="$getCreditCardBrand(row.data.attributes.brand)"
                  ...
                  <ColorLabel
                    v-if="row.data.id === defaultPaymentCard.data.id"
                    color="purple"
                    >{{ $t("global.default") }}</ColorLabel
                  >
                </div>
              </span>
            </td>
            <td>
              <span class="cell-item">
                {{ row.data.attributes.exp_month }} /
                {{ row.data.attributes.exp_year }}
              </span>
            </td>
            <td>
              <div class="action-icons">
                <label
                  class="icon-wrapper"
                  :title="$t('user_payments.set_as_default')"
                >
                  <credit-card-icon
                    v-if="row.data.id !== defaultPaymentCard.data.id"
                    size="15"
                    class="icon icon-card"
                    @click="setDefaultCard(row.data.attributes)"
                  ></credit-card-icon>
                </label>
                <label
                  class="icon-wrapper"
                  :title="$t('user_payments.delete_card')"
                >
                  <trash2-icon
                    size="15"
                    class="icon icon-trash"
                    @click="deleteCard(row.data.attributes)"
                  ></trash2-icon>
                </label>
              </div>
            </td>
        ...
        <template #empty-page>
          <InfoBox>
            <p>
              {{ $t("user_payments.empty") }}
              <router-link
                v-if="user.data.attributes.stripe_customer"
                :to="{ name: 'CreatePaymentMethod' }"
                >Add new payment method.</router-link
              >
            </p>
          </InfoBox>
        </template>
  ...
  created() {
    ...
    events.$on("action:confirmed", (data) => {
      if (data.operation === "delete-credit-card") {
        ...
        axios
          .post("/api/user/payment-cards/" + data.id, {
            _method: "delete",
          })
          .then(() => {
            // Get payments card
