    ...
    <PageTabGroup v-if="subscription && !isLoading">
      ...
      <div v-if="!subscription.data.attributes.canceled" class="state active">
        <ListInfo class="list-info">
          <ListInfoItem
            class="list-item"
            :title="$t('user_subscription.plan')"
            :content="
              subscription.data.attributes.name +
              ' - ' +
              subscription.data.attributes.capacity_formatted
            "
          />
          <ListInfoItem
            class="list-item"
            :title="$t('user_subscription.billed')"
            :content="$t('admin_page_user.subscription.interval_mo')"
          />
          <ListInfoItem
            class="list-item"
            :title="$t('user_subscription.status')"
            :content="status"
          />
          <ListInfoItem
            class="list-item capitalize"
            :title="$t('user_subscription.created_at')"
            :content="subscription.data.attributes.created_at"
          />
          <ListInfoItem
            class="list-item capitalize"
            :title="$t('user_subscription.renews_at')"
            :content="subscription.data.attributes.ends_at"
          />
        </ListInfo>
      ...
      <div v-if="subscription.data.attributes.canceled" class="state canceled">
        <ListInfo class="list-info">
          ...
          <ListInfoItem
            class="list-item capitalize"
            :title="$t('user_subscription.canceled_at')"
            :content="subscription.data.attributes.canceled_at"
          />
          <ListInfoItem
            class="list-item capitalize"
            :title="$t('user_subscription.ends_at')"
            :content="subscription.data.attributes.ends_at"
          />
        </ListInfo>
        <div class="plan-action">
...
export default {
  ...
  computed: {
    cancelButtonText() {
      return this.isConfirmedCancel
        ? this.$t("popup_share_edit.confirm")
        : this.$t("user_subscription.cancel_plan");
    },
    cancelButtonStyle() {
      return this.isConfirmedCancel ? "danger-solid" : "secondary";
    },
    resumeButtonText() {
      return this.isConfirmedResume
        ? this.$t("popup_share_edit.confirm")
        : this.$t("user_subscription.resume_plan");
    },
    resumeButtonStyle() {
