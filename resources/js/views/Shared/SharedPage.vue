<template>
  <div id="shared">
    ...
    <!--Single file page-->
    <div
      v-if="sharedDetail && sharedDetail.type === 'file' && isPageFiles"
      id="single-file"
    >
    ...
    <div
      v-if="sharedDetail && sharedDetail.type === 'folder' && isPageFiles"
      id="viewport"
    ...
    >
      <ContentSidebar v-if="navigationTree">
        ...
        <ContentGroup :title="$t('sidebar.navigator_title')" class="navigator">
          <span v-if="navigationTree.length == 0" class="empty-note navigator">
            {{ $t("sidebar.folders_empty") }}
          </span>
          <TreeMenuNavigator
            v-for="items in navigationTree"
            :key="items.unique_id"
            class="folder-tree"
            :depth="0"
            :nodes="items"
...
export default {
  ...
  computed: {
    ...mapGetters(["config", "sharedDetail", "sharedFile", "navigation"]),
    navigationTree() {
