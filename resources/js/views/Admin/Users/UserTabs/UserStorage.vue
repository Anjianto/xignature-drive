<template>
  <PageTab v-if="storage" :is-loading="isLoading" class="form-fixed-width">
    <PageTabGroup
      v-if="config.storageLimit && !user.data.attributes.subscription"
    >
      <FormLabel>
    ...
    <PageTabGroup>
      ...
      <StorageItemDetail
        ...
        :used="
          $t('storage.total_capacity', {
            capacity: storage.attributes.capacity,
          })
        "
      />
      <StorageItemDetail
        type="images"
        :title="$t('storage.images')"
        :percentage="storage.meta.images.percentage"
        :used="storage.meta.images.used"
      />
      <StorageItemDetail
        type="videos"
        :title="$t('storage.videos')"
        :percentage="storage.meta.videos.percentage"
        :used="storage.meta.videos.used"
      />
      <StorageItemDetail
        type="audios"
        :title="$t('storage.audios')"
        :percentage="storage.meta.audios.percentage"
        :used="storage.meta.audios.used"
      />
      <StorageItemDetail
        type="documents"
        :title="$t('storage.documents')"
        :percentage="storage.meta.documents.percentage"
        :used="storage.meta.documents.used"
      />
      <StorageItemDetail
        type="others"
        :title="$t('storage.others')"
        :percentage="storage.meta.others.percentage"
        :used="storage.meta.others.used"
      />
    </PageTabGroup>
  </PageTab>
</template>
...
export default {
  ...
  methods: {
    ...
    getStorageDetails() {
      axios
        .get("/api/users/" + this.$route.params.id + "/storage")
        .then((response) => {
