...
const actions = {
  ...
  restoreItem: ({ commit, getters }, item) => {
    ...
    if (!item) items = getters.fileInfoDetail;
  ...
  setDocumentSignature: ({ commit }, payload) => {
    commit("SET_DOCUMENT_SIGNATURE", payload);
  },
  closeOTP: ({ commit }) => {
...
const mutations = {
  ...
  SET_OTP_STATUS(state, status) {
    state.isOTPOpen = status;
  },
  SET_OTP(state, otp) {
    state.otp = otp;
  },
  SET_DOCUMENT_SIGNATURE(state, payload) {
