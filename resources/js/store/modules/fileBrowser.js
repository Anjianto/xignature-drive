...
const actions = {
  ...
  addFileInfoDetail: ({ commit }, fileInfoDetail) => {
    commit("ADD_FILEINFO_DETAIL", fileInfoDetail);
  },
};
...
const mutations = {
  ...
  CHANGE_ITEM_NAME(state, updatedFile) {
    ...
    state.data.find((item) => {
      if (item.unique_id == updatedFile.unique_id) {
        ...
        item.icon_color = updatedFile.icon_color
          ? updatedFile.icon_color
          : null;
        item.icon_emoji = updatedFile.icon_emoji
          ? updatedFile.icon_emoji
          : null;
      }
    });
  },
  ...
  ADD_FILEINFO_DETAIL(state, item) {
    state.fileInfoDetail.push(item);
  },
