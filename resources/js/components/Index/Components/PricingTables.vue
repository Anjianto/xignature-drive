<template>
  <div v-if="plans" class="plans-wrapper">
    <article v-for="(plan, i) in plans" :key="i" class="plan">
      <div class="plan-wrapper">
        ...
        <footer class="plan-footer">
          <b class="price">
            ...
            <small
              v-if="plan.data.attributes.tax_rates.length > 0"
              class="vat-disclaimer"
              >{{ $t("page_pricing_tables.vat_excluded") }}</small
            >
          </b>
        </footer>
      </div>
...
export default {
  ...
  created() {
    axios.get("/api/public/pricing").then((response) => {
