  ...
  <div>
    <div class="navigation-panel" v-if="fileInfoDetail[0]">
      ...
      <div class="created-at-wrapper">
        <p>
          {{ fileInfoDetail[0].filesize }}, {{ fileInfoDetail[0].created_at }}
        </p>
      </div>
...
export default {
    ...
    files() {
      ...
      this.data.map((element) => {
        if (this.fileInfoDetail[0].mimetype === "pdf") {
          if (element.mimetype === "pdf") files.push(element);
        } else {
          if (element.type === this.fileInfoDetail[0].type) files.push(element);
        }
      });
  ...
  methods: {
    ...
    shareItem() {
      let event = this.fileInfoDetail[0].shared ? "share-edit" : "share-create";
    ...
    isOTPOpen() {
      return this.$store.state.isOTPOpen;
    },
    menuOpen() {
      if (this.$isMobile()) {
