    ...
    >
      ...
      <div class="icon-item">
        ...
        <div
          ...
          :class="{
            'check-select-folder': item.type === 'folder',
            'check-select': item.type !== 'folder',
          }"
        >
        ...
        <span
          v-if="isFile || (isImage && !item.thumbnail)"
      ...
      <div class="item-name">
        ...
        <div class="item-info">
          <!--Shared Icon-->
          <div
            v-if="$checkPermission('master') && item.shared"
            class="item-shared"
          >
          ...
          <div
            v-if="$checkPermission('master') && item.user_scope !== 'master'"
            class="item-shared"
          >
      ...
      <span
        v-if="$isMobile() && !multiSelectMode && canShowMobileOptions"
        class="show-actions"
        @mousedown.stop="showItemActions"
      >
        <FontAwesomeIcon
          icon="ellipsis-h"
          class="icon-action"
  ...
  computed: {
    ...
    folderEmojiOrColor() {
      ...
      if (this.item.icon_color) {
        ...
        this.$nextTick(() => {
          this.$refs[
            `folder${this.item.unique_id}`
          ].firstElementChild.style.fill = `${this.item.icon_color}`;
        });
        return false;
      }
    ...
    isClicked() {
      return this.fileInfoDetail.some(
        (element) => element.unique_id == this.item.unique_id
      );
    },
    isFolder() {
    ...
    timeStamp() {
      return this.item.deleted_at
        ? this.$t("item_thumbnail.deleted_at", this.item.deleted_at)
    ...
    clickedItem(e) {
      ...
      if (!this.$isMobile()) {
        ...
        if (e.ctrlKey || (e.metaKey && !e.shiftKey)) {
          // Click + Ctrl
          if (
            this.fileInfoDetail.some(
              (item) => item.unique_id === this.item.unique_id
            )
          ) {
          ...
          // Click + Shift
          let lastItem = this.data.indexOf(
            this.fileInfoDetail[this.fileInfoDetail.length - 1]
          );
