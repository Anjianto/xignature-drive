<template>
  <transition name="context-menu">
    <div class="multiselect-actions" v-if="mobileMultiSelect">
      <ToolbarButton
        v-if="
          (!$isThisLocation(['trash', 'trash-root', 'shared', 'latest']) &&
      ...
      <ToolbarButton
        v-if="
          (!$isThisLocation(['shared']) && $checkPermission('master')) ||
          $checkPermission('editor')
        "
        class="action-btn"
        source="trash"
      ...
      <ToolbarButton
        v-if="!$isThisLocation(['shared'])"
        class="action-btn"
        source="download"
        :class="{ 'is-inactive': canDownloadItems }"
        :action="$t('actions.delete')"
...
export default {
  ...
  computed: {
    ...
    canDownloadItems() {
      return (
        this.fileInfoDetail.filter((item) => item.type === "folder").length !==
        0
      );
    },
  },
