      ...
      if (event.dataTransfer.items.length == 0) {
        ...
        if (
          this.fileInfoDetail.find(
            (item) => item === data && this.fileInfoDetail.length > 1
          )
        )
          return;
        ...
        if (
          ...
          this.fileInfoDetail.includes(this.draggingId)
        ) {
          this.$store.dispatch("moveItem", {
            to_item: data,
            noSelectedItem: null,
