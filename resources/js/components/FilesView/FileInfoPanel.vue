<template>
  <div v-if="fileInfoDetail.length === 1" class="file-info-content">
    <div class="file-headline" spellcheck="false">
      ...
      <div class="flex">
        ...
        <div class="file-info">
          ...
          <span v-if="fileInfoDetail[0].mimetype" class="mimetype"
            >.{{ fileInfoDetail[0].mimetype }}</span
          >
        </div>
      </div>
    </div>
    ...
    <ListInfo>
      <ListInfoItem
        v-if="fileInfoDetail[0].filesize"
      ...
      <ListInfoItem
        v-if="
          ...
          fileInfoDetail[0].user_scope !== 'master'
        "
      ...
      >
        <div class="action-button" @click="moveItem">
          <span>{{
            fileInfoDetail[0].parent
              ? fileInfoDetail[0].parent.name
              : $t("locations.home")
          }}</span>
      ...
      <ListInfoItem
        v-if="$checkPermission('master') && fileInfoDetail[0].shared"
...
export default {
  ...
  computed: {
    ...
    canShowMetaData() {
      return (
        this.fileInfoDetail[0].metadata &&
        this.fileInfoDetail[0].metadata.ExifImageWidth
      );
    },
    sharedInfo() {
