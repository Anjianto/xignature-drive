<template>
  ...
  >
    ...
    >
      ...
      <OptionGroup
        v-if="
          !$isThisLocation(['participant_uploads', 'latest']) &&
    ...
    >
      ...
      <OptionGroup v-if="!multiSelectContextMenu && item">
        ...
        <Option
          v-if="hasFolder"
          :title="$t('context_menu.no_options')"
          icon="no-options"
          class="no-options"
        />
      </OptionGroup>
...
export default {
  ...
  methods: {
    ...
    signItem() {
      const basenames = this.fileInfoDetail.reduce((acc, item) => {
