<template>
  <div>
    <ul class="meta-data-list">
      ...
      <li
        v-if="
          fileInfoDetail.metadata.ExifImageWidth &&
          fileInfoDetail.metadata.ExifImageLength
        "
      >
        <span>{{ $t("file_detail_meta.dimension") }}</span>
        <b
          >{{ fileInfoDetail.metadata.ExifImageWidth }}x{{
            fileInfoDetail.metadata.ExifImageLength
          }}</b
        >
      </li>
      ...
      <li
        v-if="
          fileInfoDetail.metadata.XResolution &&
          fileInfoDetail.metadata.YResolution
        "
      >
        <span>{{ $t("file_detail_meta.resolution") }}</span>
        <b
          >{{ fileInfoDetail.metadata.XResolution }}x{{
            fileInfoDetail.metadata.YResolution
          }}</b
        >
      </li>
      ...
      <li v-if="fileInfoDetail.metadata.GPSLongitude">
        ...
        <b>{{
          formatGps(
            fileInfoDetail.metadata.GPSLongitude,
            fileInfoDetail.metadata.GPSLongitudeRef
          )
        }}</b>
      </li>
      ...
      <li v-if="fileInfoDetail.metadata.GPSLatitude">
        ...
        <b>{{
          formatGps(
            fileInfoDetail.metadata.GPSLatitude,
            fileInfoDetail.metadata.GPSLatitudeRef
          )
        }}</b>
      </li>
    </ul>
