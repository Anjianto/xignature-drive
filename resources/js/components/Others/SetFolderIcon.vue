<template>
  <TabWrapper class="set-folder-icon">
    ...
    <TabOption
      id="emoji-list"
      :selected="true"
      :title="$t('popup_rename.tab_emoji_title')"
      icon="emoji"
    >
      <div class="select-emoji-wrapper">
        <label class="main-label"
          >{{ $t("popup_rename.select_emoji_label") }}:</label
        >

        <!-- Selected Emoji input -->
        <div
          class="select-input-wrapper"
          :class="{ 'active-menu': selectOpen }"
          @click.stop="openMenu"
        >
          <!-- If is emoji selected -->
          <div v-if="selectedEmoji" class="select-input">
            <div class="select-input-icon-wrapper" @click.stop="resetEmoji">
              <x-icon size="14" class="select-input-icon" />
            </div>
            <Emoji
              class="emoji-preview"
              :emoji="selectedEmoji"
              location="emoji-picker-preview"
            />
            <span>{{ selectedEmoji.name }}</span>
          </div>

          <!-- If is emoji not selected -->
          <div v-if="!selectedEmoji" class="not-selected">
          ...
          <div v-if="selectOpen">
            ...
            <div v-if="loadedList && emojis" class="emoji-wrapper">
              ...
              <input
                v-model="searchInput"
                class="emoji-input"
                :placeholder="$t('popup_rename.search_emoji_input_placeholder')"
                @click.stop
                @input="filterEmojis"
              />
              ...
              <ul v-show="searchInput.length < 1" class="groups-list">
                <li
                  v-for="(group, i) in emojis.emojisGroups"
                  :key="i"
                  class="group-option"
                  :class="{ active: group.name === groupInView }"
              ...
              <div v-if="searchInput.length > 0" class="group-wrapper">
                <div class="options-wrapper">
                  ...
                  <span v-if="filteredEmojis.length === 0" class="not-found">
                    {{ $t("popup_rename.emoji_list_not_found") }}</span
                  >
                </div>
              </div>
            </div>
    ...
    <TabOption :title="$t('popup_rename.tab_color_title')" icon="folder">
      <div class="color-pick-wrapper">
        <label class="main-label"
          >{{ $t("popup_rename.color_pick_label") }}:</label
        >
...
export default {
  ...
  methods: {
    checkGroupInView: debounce(function () {
      this.emojis.emojisGroups.forEach((group) => {
        let element = document
          .getElementById(`group-${group.name}`)
          .getBoundingClientRect();
        let groupBox = document
          .getElementById("group-box")
          .getBoundingClientRect();
    ...
    filterEmojis: debounce(function (emoji) {
      this.filteredEmojis = this.emojis.emojisList.filter((emoji) =>
        emoji.name.includes(this.searchInput)
      );
    }, 800),
    openMenu() {
      ...
      if (!this.selectOpen) this.loadedList = false;
  ...
  mounted() {
    ...
    this.folderData.icon_emoji
      ? (this.selectedEmoji = this.folderData.icon_emoji)
      : "";

    // If folder have already set some color set this color to selected color
    this.folderData.icon_color
      ? (this.selectedColor = this.folderData.icon_color)
      : "";
