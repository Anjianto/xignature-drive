    ...
    <div class="folder-item-wrapper">
      <div
        ...
        :class="{
          ...
          'is-dragenter': area,
          'is-inactive': disabledFolder || (disabled && draggedItem.length > 0),
        }"
        :style="indent"
        @click="getFolder"
    ...
    disabledFolder() {
      ...
      if (this.draggedItem.length > 0) {
        this.draggedItem.forEach((item) => {
          ...
          if (
            item.type === "folder" &&
            this.nodes.unique_id === item.parent_id
          ) {
            disableFolder = true;
          }
          //Disable the self folder with all children
          if (
            this.nodes.unique_id === item.unique_id &&
            item.type === "folder"
          ) {
            disableFolder = true;
    ...
    getFolder() {
      ...
      if (this.$isThisLocation("public")) {
        this.$store.dispatch("browseShared", [
          { folder: this.nodes, back: false, init: false },
        ]);
      } else {
