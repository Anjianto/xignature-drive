<template>
  <PopupWrapper name="share-edit">
    ...
    <PopupContent v-if="pickedItem && pickedItem.shared">
      ...
      <InfoBox
        v-if="sendToRecipientsMenu && isEmailSended"
      ...
      <div
        v-if="!sendToRecipientsMenu || (sendToRecipientsMenu && isEmailSended)"
        class="input-wrapper copy-input"
      >
        <label class="input-label"
          >{{ $t("shared_form.label_share_vie_email") }}:</label
        >
      ...
      >
        ...
        <div class="input-wrapper">
          ...
          <ActionButton
            v-if="
              pickedItem.shared.protected &&
              canChangePassword &&
              shareOptions.isProtected
            "
            class="change-password"
            @click.native="changePassword"
            >{{ $t("popup_share_edit.change_pass") }}</ActionButton
          >
        </div>
        ...
        <!--More options-->
        <div v-if="isMoreOptions" class="more-options">
          <!--Set expiration-->
          <div class="input-wrapper">
            <label class="input-label"
        ...
        <ActionButton
          :icon="isMoreOptions || shareOptions.expiration ? 'x' : 'pencil-alt'"
...
export default {
  ...
  computed: {
    ...
    destroyButtonText() {
      if (!this.sendToRecipientsMenu)
        return this.isConfirmedDestroy
          ? this.$t("popup_share_edit.confirm")
          : this.$t("popup_share_edit.stop");

      if (this.sendToRecipientsMenu) return this.$t("popup_share_edit.go_back");
    ...
    destroyButtonStyle() {
      ...
      if (this.sendToRecipientsMenu) return "secondary";
      return "";
    },
    secondButtonText() {
      if (!this.sendToRecipientsMenu) return this.$t("popup_share_edit.save");

      if (this.sendToRecipientsMenu)
        return this.isEmailSended
          ? this.$t("shared_form.button_done")
          : this.$t("popup_share_edit.send_to_recipients");
      return "";
    },
    isSharedLocation() {
  ...
  mounted() {
    ...
    events.$on("popup:open", (args) => {
      ...
      if (args.item.shared.expire_in) this.isMoreOptions = true;

      if (args.sentToEmail) this.sendToRecipientsMenu = true;
