  ...
  <div class="file-item" v-if="item">
    ...
    <div class="icon-item">
      ...
      <FontAwesomeIcon
        v-if="isFile || (isImage && !item.thumbnail)"
        class="file-icon"
        :class="{ 'file-icon-mobile': $isMobile() }"
        icon="file"
      />
      ...
      <FolderIcon
        v-if="isFolder"
        :item="item"
        :folder-icon="setFolderIcon"
        location="thumbnail-item"
        class="folder"
      />
    </div>
    ...
    <div class="item-name">
      ...
      <div v-if="info === 'location'">
        <span class="subtitle"
          >{{ $t("item_thumbnail.original_location") }}:
          {{ itemLocation }}</span
        >
      </div>
      ...
      <div v-if="info === 'metadata'">
        <span v-if="!isFolder" class="item-size"
          >{{ item.filesize }}, {{ item.created_at }}</span
        >
        ...
        <span v-if="isFolder" class="item-length">
          {{
            item.items == 0
              ? $t("folder.empty")
              : $tc("folder.item_counts", item.items)
          }}, {{ item.created_at }}
        </span>
      </div>
    </div>
