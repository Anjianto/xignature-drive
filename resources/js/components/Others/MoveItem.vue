  ...
  <PopupWrapper name="move">
    ...
    <PopupContent type="height-limited" v-if="pickedItem">
      ...
      <div v-if="!isLoadingTree && navigation">
        <ThumbnailItem
          v-if="fileInfoDetail.length < 2 || noSelectedItem"
        ...
        <MultiSelected
          ...
          :subtitle="
            ...
            ' ' +
            $tc('file_detail.items', this.fileInfoDetail.length)
          "
...
export default {
  ...
  methods: {
    moveItem() {
      ...
      if (
        this.fileInfoDetail.find(
          (item) => item.parent_id === this.selectedFolder.unique_id
        )
      )
        return;
      ...
      if (this.noSelectedItem) {
        this.$store.dispatch("moveItem", {
          to_item: this.selectedFolder,
          noSelectedItem: this.pickedItem,
        });
      }
      ...
      if (this.$isMobile()) events.$emit("mobileSelecting:stop");
    },
  },
  mounted() {
